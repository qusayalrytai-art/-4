...
$targetFile = Join-Path $repoRoot "index.htm.htm"
if (-not (Test-Path -LiteralPath $targetFile)) {
    Write-Host "index.htm.htm not found in $repoRoot" -ForegroundColor Red
    exit 1
}

Write-Host "Watching index.htm.htm for changes. Press Ctrl+C to stop." -ForegroundColor Cyan

$watcher = New-Object System.IO.FileSystemWatcher
$watcher.Path = $repoRoot
$watcher.Filter = "index.htm.htm"
